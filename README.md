...
Skyward 是一个 Python-first、只读、无账号的局域网 FastAPI 原型。目前仅完成 LACT 望远镜适配，并基于天体与望远镜的几何关系计算观测窗口。服务仅供局域网访问；后续目标是接入不同望远镜的实时运行状态、天体实时位置和站点天气等信息，并综合天气、天体位置以及望远镜运行/指向轨迹，判断目标适合观测的时间段。

> 当前结果仅表示基于几何关系计算的候选观测窗口，不是正式观测批准。天气、望远镜实时运行状态、机械限位、跟踪误差、实际灵敏度和联合观测条件尚未接入或评估。应用没有账号认证，仅限受控局域网访问。
...
- 2LHAASO 源：190 条，RA/Dec 按 FK5 J2000 处理；
- 可选参数：太阳最大高度、月亮最小角距、目标天顶角范围、最短连续窗口；
- GREEN/YELLOW/RED 当前几何状态；
- 当前 LACT 适配下的望远镜站点地平坐标全天图、固定指向的 FoV 圆、小型橙色太阳/月亮图标、实时/指定时刻切换和可点击源标记；
- 选定源的 8.3°局部 FoV、extension 圆、附近源、太阳/月亮方向；
- 完整源窗口本地观测计划编辑与 TXT 下载（仅当窗口内存在两个有效整秒时提供）；
- 秒级候选扫描和亚秒边界细化；单次窗口请求最多 1 天；
- HTML 页面及 `/api/v1` JSON API；
- 离线 IERS-A 数据，不在运行时联网下载。
...
当前版本只完成 LACT 适配和基于几何关系的观测窗口计算，并且仅供局域网访问。后续计划包括：

- 接入不同望远镜的实时运行状态查询；
- 提供天体实时位置、站点天气和相关环境信息；
- 纳入望远镜运行/指向轨迹、天气和天体位置；
- 综合上述信息，给出更完整、更接近实际运行条件的适合观测时间段。
...
运行时优先读取 `data/iers/finals2000A.all`，自动下载关闭。`/api/v1/health` 会报告覆盖区间和当前值是否为预测值。IERS-A 的未来部分是预测值；到期前应由维护人员从 IERS 官方来源下载新文件、离线替换并重新运行测试。

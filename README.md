# Skyward | LACT 几何观测窗口规划器

[English guide](docs/en/user-and-deployment-guide.md) | [中文手册](docs/zh/使用与部署手册.md)

Skyward 是一个 Python-first、只读、无账号的局域网 FastAPI 原型。它载入 `data/2LHAASO.txt` 的 190 个源，在 LACT/LHAASO 站点位置计算目标、太阳和月亮的几何位置，并根据可选约束生成中心窗口和完整 extension 窗口。

> 结果只表示几何候选窗口，不是正式观测批准。天气、实时设备状态、机械限位、跟踪误差、实际灵敏度和 LHAASO 联合观测条件均未评估。

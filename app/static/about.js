...
(() => {
  ...
  const copy = {
    zh: {
      ...
      alternativeCsvDownload: "下载备选源目录 CSV 示例",
      alternativeCsvBody: "格式与源表 CSV 相同，必需列为 name、ra、dec，可选列相同。在结果页的备选源目录控件中上传后，该文件将成为该计划搜索备选源的唯一目录；不会替换当前目标或已显示源表。",
      csvUploadAliases: "列名别名：source_name 对应 name，ra_deg 对应 ra，dec_deg 对应 dec，extension_deg 对应 ext。",
      csvUploadLimits: "限制：每个文件最多 1,000 行、512 KiB。源名需唯一（不区分大小写）且长度为 1–80 个字符；坐标必须为有限数值，满足 0 ≤ RA < 360、−90 ≤ Dec ≤ 90。扩展半径或不确定度不能为负数。不得使用尚未公开的 2LHAASO 保留名称。",
    ...
    en: {
      ...
      alternativeCsvDownload: "Download alternative-catalogue CSV example",
      alternativeCsvBody: "Use the same format as a source catalogue: name, ra, dec are required and the same optional columns are accepted. Upload it through the alternative-source controls on a result page. It becomes the exclusive candidate pool for alternatives in that plan; it does not replace the target or displayed catalogues.",
      csvUploadAliases: "Column aliases: source_name maps to name, ra_deg to ra, dec_deg to dec, and extension_deg to ext.",
      csvUploadLimits: "Limits: up to 1,000 rows and 512 KiB per file. Names must be unique (case-insensitive) and 1–80 characters; coordinates must be finite with 0 ≤ RA < 360 and −90 ≤ Dec ≤ 90. Negative extension or uncertainty values are rejected. The reserved unpublished 2LHAASO name cannot be used.",

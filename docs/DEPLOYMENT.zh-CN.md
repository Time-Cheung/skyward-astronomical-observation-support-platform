...
当前 V2.2（2.2.20260924）没有账号、租户隔离或授权，禁止直接暴露公网。优先只监听回环地址；局域网部署必须由管理员限制可信源 CIDR，并验证未批准网络不可连接。只有服务器安装 Python 依赖，客户端只需要浏览器。
...
V2.2（2.2.20260924）包含源表图标、批量目标列表计算、跟踪视场曲线、窗口排序和计划预览排序。CSV 格式仍在数据说明页提供，均使用 UTF-8，源表至少需要 `name,ra,dec`，批量目标列表需要 `name,ra,dec,ext`。
